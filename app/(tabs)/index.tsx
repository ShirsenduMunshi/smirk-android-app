...
export default function Index() {
  ...
  useEffect(() => {
    const hideBar = async () => {
      ...
      await NavigationBar.setBehaviorAsync('overlay-swipe'); // Optional: allows swipe to reveal
    }
    hideBar();
  }, []);
        ...
        <LinearGradient colors={colors.gradients.surface} style={homeStyles.todoItem} start={{x:0, y:0}} end={{x:1,y:1}}>
            <TouchableOpacity style={homeStyles.checkbox} activeOpacity={0.7} onPress={()=> handleToggleTodo(item._id)}>
              <LinearGradient colors={item.isCompleted ? colors.gradients.success : colors.gradients.muted } style={[homeStyles.checkboxInner, {borderColor: item.isCompleted ? "transparent" : colors.border}]}>
                  {item.isCompleted &&  <Ionicons name='checkmark' color={'#fff'} size={28} />}
              </LinearGradient>
            </TouchableOpacity>
            {isEditing ? (
              <View style={homeStyles.editContainer}>
                ...
                <View style={homeStyles.editButtons}>
                  <TouchableOpacity onPress={handleSaveEdit} activeOpacity={0.6}>
              ...
              <Text style={[homeStyles.todoText, item.isCompleted && ({textDecorationLine: "line-through", color: colors.textMuted, opacity: 0.6})]}>{item.text}</Text>
  ...
  return (
    <LinearGradient colors={colors.gradients.background} style={homeStyles.container}>
      <StatusBar hidden={true} barStyle={colors.statusBarStyle} />
